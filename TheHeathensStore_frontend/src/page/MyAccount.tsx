...
import {changePassword, createShippingAddress, deleteShippingAddress, getShippingAddresses, updateProfile, updateShippingAddress} from '../service/accountService.ts';
...
const MyAccount = () => {
    ...
    const addAddress = (event: FormEvent) => {
        event.preventDefault();
        void runAction(async () => {
            await createShippingAddress({recipientName: addressName, recipientPhone: addressPhone, address: addressValue, isDefault: newAddressIsDefault || addresses.length === 0});
            setAddresses(await getShippingAddresses());
            setAddressName(''); setAddressPhone(''); setAddressValue('');
            setNewAddressIsDefault(false);
        }, 'Shipping address added.');
    };

    const beginEditAddress = (item: ShippingAddress) => {
        setEditingAddressId(item.id);
        setEditAddressName(item.recipientName);
        setEditAddressPhone(item.recipientPhone);
        setEditAddressValue(item.address);
        setEditAddressIsDefault(item.isDefault);
    };
    ...
    const saveAddress = (event: FormEvent) => {
        ...
        void runAction(async () => {
            await updateShippingAddress(editingAddressId, {
                recipientName: editAddressName,
                recipientPhone: editAddressPhone,
    ...
    const makeDefaultAddress = (item: ShippingAddress) => {
        void runAction(async () => {
            await updateShippingAddress(item.id, {
    ...
    return (
        <section className="bg0 p-t-75 p-b-120">
            <div className="container">
                ...
                <section className="account-card bor10 p-all-30 m-b-30">
                    <h2 className="mtext-109 p-b-20">Shipping addresses</h2>
                    <div className="row">
                        {addresses.map((item) => (
                            <div key={item.id} className="col-md-6 p-b-15">
                                <div className="bor1 p-all-15 h-full">
                                    {editingAddressId === item.id ? (
                                        <form onSubmit={saveAddress}>
                                            <input className="form-control m-b-10" aria-label="Recipient name" value={editAddressName} onChange={(event) => setEditAddressName(event.target.value)} required/>
                                            <input className="form-control m-b-10" aria-label="Recipient phone" value={editAddressPhone} onChange={(event) => setEditAddressPhone(event.target.value)} required/>
                                            <textarea className="form-control m-b-10" aria-label="Shipping address" value={editAddressValue} onChange={(event) => setEditAddressValue(event.target.value)} required/>
                                            <label className="d-block p-b-10"><input type="checkbox" className="m-r-6" checked={editAddressIsDefault} onChange={(event) => setEditAddressIsDefault(event.target.checked)} disabled={item.isDefault}/>Default address</label>
                                            <button className="btn btn-sm btn-dark m-r-8">Save</button>
                                            <button type="button" className="btn btn-sm btn-outline-secondary" onClick={() => setEditingAddressId(null)}>Cancel</button>
                                        </form>
                                    ) : (
                                        <>
                                            <strong>{item.recipientName} {item.isDefault && <span className="badge badge-dark">Default</span>}</strong>
                                            <p>{item.recipientPhone}</p>
                                            <p>{item.address}</p>
                                            <button type="button" className="btn btn-sm btn-outline-dark m-r-8" onClick={() => beginEditAddress(item)}>Edit</button>
                                            {!item.isDefault && <button type="button" className="btn btn-sm btn-outline-primary m-r-8" onClick={() => makeDefaultAddress(item)}>Set default</button>}
                                            <button type="button" className="btn btn-sm btn-outline-danger" onClick={() => void runAction(async () => {await deleteShippingAddress(item.id); setAddresses(await getShippingAddresses());}, 'Address removed.')}>Remove</button>
                                        </>
                                    )}
                                </div>
                            </div>
                    ...
                    <form className="row p-t-15" onSubmit={addAddress}>
                        <div className="col-md-3"><input className="form-control" placeholder="Recipient name" value={addressName} onChange={(event) => setAddressName(event.target.value)} required/></div>
                        <div className="col-md-3"><input className="form-control" placeholder="Phone" value={addressPhone} onChange={(event) => setAddressPhone(event.target.value)} required/></div>
                        <div className="col-md-4"><input className="form-control" placeholder="Shipping address" value={addressValue} onChange={(event) => setAddressValue(event.target.value)} required/></div>
                        <div className="col-md-2"><button className="btn btn-dark w-full">Add address</button></div>
                        <div className="col-12 p-t-10"><label><input type="checkbox" className="m-r-6" checked={newAddressIsDefault} onChange={(event) => setNewAddressIsDefault(event.target.checked)}/>Make this my default address</label></div>
                    </form>
                </section>
