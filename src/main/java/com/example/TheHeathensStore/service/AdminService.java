...
public class AdminService {
    ...
    public Map<String, Object> getDashboard() {
        ...
        long lowStock = products.stream()
                                .filter(product -> product.isActive() && product.getStock() <= LOW_STOCK_THRESHOLD)
                                .count();
        ...
        orders.stream()
              ...
              .forEach(order -> revenueByMonth.merge(YearMonth.from(order.getCreatedAt())
                                                              .toString(), order.getTotalAmount(), BigDecimal::add));

        Map<String, Object> result = new LinkedHashMap<>();
        result.put("revenue", revenue);
        result.put("orders", orders.size());
        result.put("pendingOrders", pendingOrders);
        result.put("customers", users.stream()
        ...
        result.put("topProducts", products.stream()
                                          .sorted(Comparator.comparing(Product::getSalesCount, Comparator.nullsFirst(Long::compareTo))
                                                            .reversed())
                                          .limit(5)
                                          .map(this::productRow)
                                          .toList());
    ...
    public List<Map<String, Object>> getProducts() {
        return productRepository.findAll()
                                .stream()
                                .sorted(Comparator.comparing(Product::getCreatedAt, Comparator.nullsLast(LocalDateTime::compareTo))
                                                  .reversed())
                                .map(this::productRow)
                                .toList();
    }
    ...
    public Map<String, Object> getCatalog() {
        Map<String, Object> result = new LinkedHashMap<>();
        result.put("teams", teamRepository.findAll()
                                          .stream()
                                          .map(team -> {
                                              Map<String, Object> row = new LinkedHashMap<>();
                                              row.put("id", team.getId());
                                              row.put("name", team.getName());
                                              row.put("type", team.getTeamType()
                                                                  .name());
                                              row.put("leagueId", team.getLeague() == null ? null : team.getLeague()
                                                                                                        .getId());
                                              row.put("leagueName", team.getLeague() == null ? null : team.getLeague()
                                                                                                          .getName());
                                              return row;
                                          })
                                          .toList());
        result.put("seasons", seasonRepository.findAll()
                                              .stream()
                                              .map(season -> Map.of("id", season.getId(), "name", season.getName()))
                                              .toList());
        result.put("leagues", leagueRepository.findAll()
                                              .stream()
                                              .map(league -> Map.of("id", league.getId(), "name", league.getName()))
                                              .toList());
        result.put("jerseyTypes", Arrays.stream(Product.JerseyType.values())
                                        .map(Enum::name)
                                        .toList());
        return result;
    }
    ...
    public List<Map<String, Object>> getOrders() {
        ...
        return orderRepository.findAllByOrderByCreatedAtDesc()
                              .stream()
                              .map(order -> orderRow(order, users.get(order.getUserId())))
                              .toList();
    }
    ...
    public Map<String, Object> updateOrder(UUID uuid, AdminOrderUpdateRequest request) {
        ...
        try {
            if (request.orderStatus() != null && !request.orderStatus()
                                                         .isBlank()) {
                order.setOrderStatus(Order.OrderStatus.valueOf(request.orderStatus()
                                                                      .trim()
                                                                      .toUpperCase(Locale.ROOT)));
            }
            if (request.paymentStatus() != null && !request.paymentStatus()
                                                           .isBlank()) {
        ...
        order.setInternalNote(request.internalNote() == null ? null : request.internalNote()
                                                                             .trim());
        Order saved = orderRepository.saveAndFlush(order);
        return orderRow(saved, userRepository.findById(saved.getUserId())
                                             .orElse(null));
    }
    ...
    public List<Map<String, Object>> getCustomers() {
        ...
        Map<Long, UserInfo> infoByUser = userInfoRepository.findAll()
                                                           .stream()
                                                           .collect(Collectors.toMap(UserInfo::getUserId, Function.identity()));
        return userRepository.findAll()
                             .stream()
                             .filter(user -> !isManagementAccount(user.getId()))
                             .sorted(Comparator.comparing(User::getCreatedAt, Comparator.nullsLast(LocalDateTime::compareTo))
                                               .reversed())
    ...
    public Map<String, Object> addStaffMember(AdminStaffRequest request) {
        ...
        Staff staff = staffRepository.saveAndFlush(Staff.builder()
                                                        ...
                                                        .build());
        return staffRow(staff, user, userInfoRepository.findByUserId(user.getId())
                                                       .orElse(null));
    }
    ...
    public Map<String, Object> updateCustomerStatus(UUID uuid, boolean active) {
        ...
        return customerRow(user, userInfoRepository.findByUserId(user.getId())
                                                   .orElse(null), orders);
    }
    ...
    public String exportProductsCsv() {
        ...
        for (Product product : productRepository.findAll()) {
            csv.append(product.getUuid())
               .append(',')
               .append(csvValue(product.getName()))
               .append(',')
               .append(product.getPrice())
               .append(',')
               .append(product.getStock())
               .append(',')
               .append(product.getSalesCount())
               .append(',')
               .append(product.getJerseyType())
               .append(',')
               .append(csvValue(product.getTeam() == null ? "" : product.getTeam()
                                                                        .getName()))
               .append(',')
               .append(csvValue(product.getSeason() == null ? "" : product.getSeason()
                                                                          .getName()))
               .append(',')
    ...
    public Map<String, Object> saveSetting(StoreSettingRequest request) {
        ...
        if (key.startsWith("shipping.")) {
            try {
                if (new BigDecimal(request.value()
                                          .trim()).signum() < 0) throw new NumberFormatException();
        ...
        setting.setSettingValue(request.value()
                                       .trim());
        setting.setDescription(request.description() == null ? "" : request.description()
                                                                           .trim());
    ...
    public void deleteSetting(String key) {
        StoreSetting setting = storeSettingRepository.findBySettingKey(key.trim()
                                                                          .toLowerCase(Locale.ROOT))
    ...
    public List<Map<String, Object>> getRecords() {
        return adminRecordRepository.findAllByOrderByUpdatedAtDesc()
                                    .stream()
                                    .map(this::recordRow)
                                    .toList();
    }
    ...
    public Map<String, Object> saveRecord(UUID uuid, AdminRecordRequest request) {
        ...
        try {
            record.setRecordType(AdminRecord.RecordType.valueOf(request.type()
                                                                       .trim()
                                                                       .toUpperCase(Locale.ROOT)));
        } catch (IllegalArgumentException error) {
            throw new InvalidRequestException("Invalid management record type");
        }
        if (request.startsAt() != null && request.endsAt() != null && request.endsAt()
                                                                             .isBefore(request.startsAt())) {
            throw new InvalidRequestException("End time must be after start time");
        }
        record.setRecordKey(request.key()
                                   .trim());
        record.setTitle(request.title()
                               .trim());
        record.setContent(request.content() == null ? "" : request.content()
                                                                  .trim());
        record.setRecordValue(request.value() == null ? "" : request.value()
                                                                    .trim());
    ...
    private void applyProduct(Product product, AdminProductRequest request) {
        ...
        try {
            jerseyType = Product.JerseyType.valueOf(request.jerseyType()
                                                           .trim()
                                                           .toLowerCase(Locale.ROOT));
        } catch (IllegalArgumentException error) {
    ...
    private Map<String, Object> productRow(Product product) {
        ...
        row.put("jerseyType", product.getJerseyType()
                                     .name());
        row.put("teamId", product.getTeam() == null ? null : product.getTeam()
                                                                    .getId());
        row.put("teamName", product.getTeam() == null ? null : product.getTeam()
                                                                      .getName());
        row.put("leagueName", product.getTeam() == null || product.getTeam()
                                                                  .getLeague() == null ? null : product.getTeam()
                                                                                                       .getLeague()
                                                                                                       .getName());
        row.put("seasonId", product.getSeason() == null ? null : product.getSeason()
                                                                        .getId());
        row.put("season", product.getSeason() == null ? null : product.getSeason()
                                                                      .getName());
        row.put("active", product.isActive());
        row.put("createdAt", product.getCreatedAt());
        row.put("updatedAt", product.getUpdatedAt());
        row.put("thumbnailUrl", productImageRepository.findByProductIdAndIsThumbnailTrue(product.getId())
    ...
    private Map<String, Object> orderRow(Order order, User user) {
        ...
        row.put("paymentStatus", order.getPaymentStatus()
                                      .name());
        row.put("shippingMethod", order.getShippingMethod()
                                       .name());
    ...
    private Map<String, Object> recordRow(AdminRecord record) {
        Map<String, Object> row = new LinkedHashMap<>();
        row.put("uuid", record.getUuid());
        row.put("type", record.getRecordType()
                              .name());
        row.put("key", record.getRecordKey());
        row.put("title", record.getTitle());
        row.put("content", record.getContent());
        row.put("value", record.getRecordValue());
        row.put("active", record.isActive());
        row.put("startsAt", record.getStartsAt());
        row.put("endsAt", record.getEndsAt());
